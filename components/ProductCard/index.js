...
export default function ProductCard({
  ...
  return <div style={{ border: "none", position: "relative", cursor: "pointer", maxWidth: width }} width="100%" >
    <div style={{ position: "absolute", zIndex: 2, top: 5, right: 0 }}>
      <Button onClick={(e) => onActionClick()} type='abort' iconRight={icon} px={0.6} auto />
    </div>
    <Link href={`/products/${data?.id}`} passHref={false} width='100%' style={{ display: "flex", width: width, margin: 10, justifyContent: "center", alignItems: "center" }} >
      <div>
